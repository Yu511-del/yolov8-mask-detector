...
|:---|:---:|---:|---:|---:|---:|:---:|
| `scale_n` (YOLOv8n) | 3.2M | 0.8598 | 0.5804 | 0.8494 | 0.7873 | 40.3 |
| `scale_s` (YOLOv8s) | 11.2M | 0.8115 | 0.5910 | 0.9082 | 0.7487 | — |
| `scale_m` (YOLOv8m) | 25.9M | **0.8621** | **0.6003** | 0.8764 | 0.7835 | 41.8 |

> ¹ FPS 于 NVIDIA RTX 4090 测得。nano 与 medium 速度相近（GPU 饱和）；边缘设备上 nano 优势将显著拉开。
>
> **BestModel = YOLOv8m（精度最优）；性价比首选 = YOLOv8n（仅差 0.23% mAP50，参数少 8 倍）**
...
YOLOv8m 以 25.9M 参数取得最高精度（mAP@50=0.8621），但 YOLOv8n 仅以 3.2M 参数（少 8 倍）达到 0.8598，差距仅 0.23%。RTX 4090 上两者推理速度接近（~40 FPS），而在边缘设备上 nano 的轻量优势将显著体现。small 模型的异常进一步说明：小数据集上模型选择需与数据量匹配。
...
3. **YOLOv8m 精度最优**（mAP@50=0.8621），**YOLOv8n 性价比最高**（仅差 0.23%，参数少 8 倍）
