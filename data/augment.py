...
def _copy_split(src_img_dir, src_label_dir, dst_img_dir, dst_label_dir):
    """
    Copy an existing YOLO split into a destination split directory.
    """
    ...
    for src_txt in src_label_dir.glob("*.txt"):
        shutil.copy2(src_txt, dst_label_dir / src_txt.name)
...
if __name__ == "__main__":
    ...
    # Materialize the expected YOLO split structure for training.
    augmented_root = project_root / "data" / "augmented"
    _copy_split(
        train_img_orig,
        train_label_orig,
        augmented_root / "images" / "train",
        augmented_root / "labels" / "train",
    )
    _copy_split(
        project_root / "data" / "processed" / "images" / "val",
        project_root / "data" / "processed" / "labels" / "val",
        augmented_root / "images" / "val",
        augmented_root / "labels" / "val",
    )
    _copy_split(
        project_root / "data" / "processed" / "images" / "test",
        project_root / "data" / "processed" / "labels" / "test",
        augmented_root / "images" / "test",
        augmented_root / "labels" / "test",
    )